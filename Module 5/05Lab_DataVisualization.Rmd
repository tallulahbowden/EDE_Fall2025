...
ggplot(
  PeterPaul.chem.nutrients,
  aes(x = factor(month, 1:12, labels = month.abb))) +
  labs(x = "Month")+
  labs(y = "Lake Observations")+
  geom_bar()
...
temperature_date_plot <- ggplot(
  ...
  scale_x_date(
    date_breaks = "5 years",
    date_labels = "%Y") +
  labs(title = "Temperature by Date across Depths")
  

temperature_date_plot
...
AQI_plot <- EPAair %>%
  ...
  labs(color = "Latitude") +
  labs(title = "AQI Values across Latitude")+
...
PM25_year <- EPAair %>%
  ...
  scale_x_discrete(
    name = "Year",
    drop = F)+
  labs(title = "PM2.5 Concentrations by Year")
...
ggplot(Neonics, aes(x = reorder(Endpoint, Endpoint,length))) +
  geom_bar()+
  theme(axis.text.x = element_text(angle = 45,  hjust = 1))+
  labs(x = "Endpoint") +
  labs( y = "Total") +
  labs(title = "Total Number of Each Endpoint")
