...
#Install packages
library(tidyverse) #includes dplyr, ggplot, and other packages
library(lubridate)
...
-   The 1st parameter of the filter command is the dataframe we wish to filter. 
-   The 2nd one is the **filter expression**:
...
NTL.phys.data.TuesdayLake <- 
  filter(NTL.phys.data, lakename %in% c("Tuesday Lake"))
NTL.phys.data.TuesdayLake <-
  filter(NTL.phys.data.TuesdayLake, year4 >= 1990 & year4 <= 1999)

```
Question: Why don't we filter using row numbers?

> Answer: Those row numbers can be changed very easily and don't mean anything 
...
NTL.phys.data.Tuesday.july <- NTL.phys.data %>%
  mutate(month = month(sampledate)) %>%
  filter(lakename == "Tuesday Lake") %>%
  filter(year4 >= 1990 & year4 <= 1999) %>%
  filter(month == 7) %>%
  select(lakename, year4, month, daynum:comments)
...
NTL.phys.data.Tuesday.july.watertemp <- 
  filter(NTL.phys.data.Tuesday.july, depth == 0)
summarise(NTL.phys.data.Tuesday.july.watertemp$temperature_C)
class(NTL.phys.data.Tuesday.july.watertemp$temperature_C)
```
...
NTL.phys.data.gathered <- pivot_longer(
  NTL.phys.data, 
  cols = irradianceWater:irradianceDeck, 
...
NTL.phys.data.spread <- pivot_wider(
