...
huc8_filter <- filter(huc8_sf, SUBBASIN == "Upper Neuse")

#Create a map
ggplot()+
  geom_sf(data = huc8_sf, fill = 'lightgrey', color = 'grey') +
  geom_sf(data = huc8_filter, fill = 'yellow', color = 'blue')
...
enviroatlas <- read.csv(
  ...
  group_by(HUC_8)%>%
  summarise(sum_avg_wind = sum(AvgWindEnergy))

  
#Join to HUC_8 features
wind_sf_join <- huc8_sf %>% 
  left_join(
    enviroatlas, 
    by = c("HUC_8" = "HUC_8") 
    )%>%
...
#View the outputs
ggplot() + 
  geom_sf(data = wind_sf_join, aes(fill = sum_avg_wind))+
  labs(fill = "MW/Year")+
  labs(title = "Wind Energy by HUC 8")
...
#Map the data
ggplot()+
  geom_sf(data = huc2_sf, aes(fill = ACRES))
...
myMap2 + mapview(triCo_intersect, alpha.regions = 0)

#Intersect the HUC_8s
triCo_huc8_sf <- triCo %>% st_intersection(huc8_sf_utm)
myMap2 + mapview(triCo_huc8_sf, alpha.regions = 0)
triCo_huc8_sf <- triCo_huc8_sf %>% 
  mutate(Area_m2 = as.numeric(st_area(triCo_huc8_sf$geometry)))
  
mapview(triCo_huc8_sf, zcol='Area_m2')
...
resultMask <- st_intersects(counties_sf_utm,
                            the_site_counties,
                            sparse = F)
selCounties2 <- counties_sf_utm[resultMask,]

mapview(selCounties2) + mapview(the_site_counties)
