...
upper_neuse_sf <- st_read(here("./Data/Spatial/NCHUC8.shp")) %>% 
...
ggplot() + 
  geom_sf(data = huc8_sf,color='gray') + 
  geom_sf(data = upper_neuse_sf, color='blue', fill='yellow')
```

## 2.3 Reading in GeoJSON data -- WE DISCUSS THIS IN THE SYNCRHONOUS SECTION

More and more spatial data are appearing on-line, in a format that allows us to connect directly to the data vs downloading local copies to our workspace. An example is the Homeland Infrastructure Foundation-Level Data (HIFLD) Their open data site (<https://hifld-geoplatform.hub.arcgis.com/pages/hifld-open>) has links to many datasets.
...
When the data is served in GeoJSON format, we can ingest it directly in to R. Follow these steps:
  ...
  - Scroll down to the Open Data Categories area. Select Energy (for example)
...
```{r load.geojson.data}
#Create the data source URL string
data_source_URL <- paste0(
  'https://services1.arcgis.com/Hp6G80Pky0om7QvQ/arcgis/rest/services/',
  'Plants_gdb/FeatureServer/0/',
  'query?outFields=*&where=1%3D1&f=geojson'
)

#Read the data from the URL into R as a dataframe
powerplants_sf <- st_read(data_source_URL)
...
#View the outputs
ggplot() + 
  geom_sf(data=huc8_wind_sf,aes(fill=WindEnergy))+
  labs(
...
#Map the data
ggplot() + geom_sf(data=huc8_sf,aes(fill=ACRES))
...
#View the result
myMap + mapview(neuse_intersect, alpha.regions = 0)
...
huc8_sf_utm %>% st_filter(triCo) %>% mapview() 


#Intersect the HUC_8s
huc8_sf_utm %>% st_intersection(triCo) %>% mapview()
