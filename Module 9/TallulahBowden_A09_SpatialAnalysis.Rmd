...
```{r Read the county data into an sf dataframe, warning=F, message=F}
...
> ANSWER: The EPSG code of the Counties dataset is 4269. This is a geographic 
coordinate reference system. It uses angular coordinate units and is associated
with the North American Datum 1983.
> 
...
```{r Read in USGS Gage Locations, warning=F, message=F}
...
> ANSWER: The column labeled "dec_long_va" contains x coordinate values. The
column labeled "dec_lat_va" contains y coordinate values.
...
```{r Convert dataframe to sf dataframe, warning=F, message=F}
...
tells us what type of geometry each row is (point, line, or polygon) and also
contains its geographic location as (x, y) coordinates. The columns labeled
'dec_long_va' and 'dec_lat_va' have disappeared since those were used to create
the (x, y) coordinates.
...
```{r Plot the spatial features, warning=F, message=F}
...
```{r Read in gage site data, warning=F, message=F}
...
```{r spatial selection, warning=F, message=F}
