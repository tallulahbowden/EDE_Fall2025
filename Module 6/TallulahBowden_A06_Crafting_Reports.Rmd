...
```{r setup, warning = F, include = F, echo = F}
#Load packages
library(tidyverse);library(here);library(lubridate)

#Load data
epa <- read.csv(
  here("Processed/EPAair_O3_PM25_NC1819_Processed.csv"),
  stringsAsFactors = TRUE)

#Fix dates
epa$Date = ymd(epa$Date)
```
...
```{r project.setup, message=FALSE, warning=FALSE, echo=T}
#Import libraries
library(tidyverse)
library(lubridate)
library(here)
...
```{r data.summary, echo = F, message = F, warning = F}
...
```{r, ozone boxplot 2018, echo = F, message = F, warning= F, fig.align='left', fig.cap = "Monthly ozone concentrations from 2018", fig.dim = c(8,4)}
ozone_2018 <- epa_data %>%
  filter(Year == 2018) %>%
  mutate(month.name.abb = month(Date, label = T)) %>%
  ggplot(mapping = 
           aes(x = month.name.abb,
               y = Ozone))+
  geom_boxplot()+
  labs(x = "Month")+
...
```{r, ozone boxplot 2019, echo = F, message = F, warning = F, fig.align='left', fig.cap = "Monthly ozone concentrations from 2019", fig.dim = c(8,4)}
ozone_2019 <- epa_data %>%
  filter(Year == 2019) %>%
  mutate(month.name.abb = month(Date, label = T)) %>%
  ggplot(mapping = 
...
## Git Repository
You can find my GitHub Repository at this [link](https://github.com/tallulahbowden/EDE_Fall2025).
I did not use AI in the completion of this assignment.
Tallulah Bowden, 10/18/2025
